...
beforeEach(async () => {
  ...
  // One test exercises a debug endpoint, which now requires an admin.
...
test("create subtask returns 201 and task with subtask id", async () => {
  ...
  const response = resAuth.body as JsonRecord & {
    sub_tasks: unknown[];
    side_quests: unknown[];
  };
...
test("completing a subtask grants XP once", async () => {
  ...
  const subComplete = await client.patch(
    `/api/tasks/${taskId}/subtasks/${subId}/status`,
    {
      body: { status: "done" },
      headers: {
        authorization: `Bearer ${authToken}`,
...
test("re-completing a subtask does not grant additional XP", async () => {
  ...
  const subTasks =
    (subRes.body as JsonRecord & { sub_tasks: Array<JsonRecord> }).sub_tasks ||
    [];
  expect(subTasks.length).toBeGreaterThan(0);
  const subId = subTasks[0].id as number;

  const firstComplete = await client.patch(
    `/api/tasks/${taskId}/subtasks/${subId}/status`,
    {
      body: { status: "done" },
      headers: {
  ...
  const secondComplete = await client.patch(
    `/api/tasks/${taskId}/subtasks/${subId}/status`,
    {
      body: { status: "done" },
      headers: {
