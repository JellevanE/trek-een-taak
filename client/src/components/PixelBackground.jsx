...
export default function PixelBackground({ showControls = false }) {
  ...
  useEffect(() => {
    ...
    // No 2d context (e.g. jsdom in tests, or canvas unsupported) — skip the
    // animation entirely rather than throwing on the first draw call.
    ...
    const tick = () => {
      ...
      for (const p of particlesRef.current) {
        ...
        const angle = (Math.sin(p.x * fieldScale + time + p.phase) +
