...
test("campaign sidebar filters quests and fetches scoped tasks", async () => {
  const campaigns = [
    {
      id: 1,
      name: "Campaign Alpha",
      progress_summary: "1/2",
      stats: { quests_completed: 1, quests_total: 2 },
    },
    {
      id: 2,
      name: "Campaign Beta",
      progress_summary: "0/1",
      stats: { quests_completed: 0, quests_total: 1 },
    },
  ];
  const quests = [
    {
      id: 10,
      description: "Alpha quest",
      status: "todo",
      priority: "medium",
      task_level: 1,
      campaign_id: 1,
  ...
  setupAuthenticatedApp((url, options = {}) => {
    if (url === "/api/users/me") {
      return okResponse({ user: { rpg: { level: 1 } } });
    }
    if (url === "/api/campaigns") return okResponse({ campaigns });
    if (url === "/api/tasks") return okResponse({ tasks: quests });
    if (url === "/api/tasks?campaign_id=1") {
      return okResponse({ tasks: filteredQuests });
    }
  ...
  // Sidebar defaults to collapsed; expand it to reach the full campaign list.
  fireEvent.click(screen.getByLabelText("Expand campaigns panel"));
  const sidebar = screen.getByRole("complementary", { name: /campaigns/i });
  const campaignButton = within(sidebar).getByRole("button", {
...
test("creating a quest submits selected campaign id", async () => {
  ...
  await waitFor(() =>
    expect(screen.getByRole("button", { name: /Add Quest/i }))
      .toBeInTheDocument()
  );
  ...
  const select = screen.getByRole("combobox", {
    name: /Assign quest to campaign/i,
  });
...
test("allows creating a campaign from the sidebar", async () => {
  ...
  setupAuthenticatedApp((url, options = {}) => {
    ...
    if (
      url === "/api/campaigns" && (!options.method || options.method === "GET")
    ) {
  ...
  await waitFor(() =>
    expect(screen.getByLabelText("Expand campaigns panel")).toBeInTheDocument()
  );
  // Sidebar defaults to collapsed; expand it to reach the "New Campaign" action.
  fireEvent.click(screen.getByLabelText("Expand campaigns panel"));

  fireEvent.click(screen.getByRole("button", { name: /New Campaign/i }));

  fireEvent.change(screen.getByLabelText(/Name/i), {
    target: { value: "Skyward League" },
  });
...
test("allows editing the selected campaign", async () => {
  ...
  setupAuthenticatedApp((url, options = {}) => {
    ...
    if (url.startsWith("/api/tasks?")) {
      return okResponse({
        tasks: quests.filter((q) =>
          String(q.campaign_id) === url.split("=").pop()
        ),
      });
    }
  ...
  // Sidebar defaults to collapsed; expand it to select the campaign.
  fireEvent.click(screen.getByLabelText("Expand campaigns panel"));

  const sidebar = screen.getByRole("complementary", { name: /campaigns/i });
  const campaignButton = await within(sidebar).findByRole("button", {
    name: /Original Name/i,
  });
  fireEvent.click(campaignButton);

  // "Edit Selected" opens the (lazy) campaign detail page; its Settings tab
  // hosts the edit form. Clicking the tab loads the form with current values.
  ...
  // Scope to the detail page — the sidebar also shows an inline form in edit mode.
  const detailPage = settingsTab.closest(".campaign-detail-page");
  const nameInput = within(detailPage).getByLabelText(/Name/i);
  fireEvent.change(nameInput, { target: { value: "Renamed Campaign" } });

  fireEvent.click(within(detailPage).getByRole("button", { name: /^Save$/i }));

  await waitFor(() =>
    expect(updatedPayload).toMatchObject({ name: "Renamed Campaign" })
  );
  await waitFor(() =>
    expect(within(sidebar).getByText("Renamed Campaign")).toBeInTheDocument()
  );
});
...
test("add side quest button works even when quest already has side quests", async () => {
  ...
  setupAuthenticatedApp((url, options = {}) => {
    if (url === "/api/users/me") {
      return okResponse({ user: { rpg: { level: 3 } } });
    }
    if (url === "/api/campaigns") return okResponse({ campaigns: [] });
    if (url === "/api/tasks" && (!options.method || options.method === "GET")) {
      return okResponse({ tasks: quests });
  ...
  await waitFor(() =>
    expect(screen.getByText("Master quest")).toBeInTheDocument()
  );

  const questCard = screen.getByText("Master quest").closest(".quest");
  expect(questCard).toBeTruthy();
  const addButton = within(questCard).getByRole("button", {
    name: /\+ Add Side Quest/i,
  });
  fireEvent.click(addButton);

  await waitFor(() => {
    expect(screen.getByPlaceholderText(/Add a side-quest/i))
      .toBeInTheDocument();
  });
});
