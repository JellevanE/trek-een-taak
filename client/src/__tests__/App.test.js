...
describe("App", () => {
  ...
  test("shows and hides keyboard shortcuts", () => {
    ...
    expect(
      screen.getByText("Keep your hands on the keys to fly through quests."),
    )
      .toBeInTheDocument();
    fireEvent.click(screen.getByText("Close"));
    expect(
      screen.queryByText("Keep your hands on the keys to fly through quests."),
    ).not
      .toBeInTheDocument();
  });
  ...
  describe("Campaign Sidebar", () => {
    ...
    test("collapses and expands the sidebar", () => {
      ...
      expect(setCampaignSidebarCollapsed).toHaveBeenCalledWith(
        expect.any(Function),
      );
    });
