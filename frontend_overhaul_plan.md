...
- [x] **Follow-up · Hook split enforcement (urgent):** Finish extracting `useQuestSelection`, `useQuestInteractions`, and `useQuestAnimations` so `useQuests.js` only stitches their outputs together. The current 700+ line hook still owns selection, keyboard shortcuts, undo queues, and animation flags, so re-renders remain noisy until this split lands. _Completed via the dedicated hooks under `client/src/features/quest-board/hooks/`, with `client/src/hooks/useQuests.js` now acting as the orchestrator._
- [x] **Follow-up · Hook-level tests:** Add RTL/Jest coverage for the new hooks (selection reset, undo timer expiry, layout refresh scheduling, collapse timers) and make sure the existing `useQuestData` tests stay green. These tests were part of the original acceptance criteria and should block the story from exiting. _Coverage lives beside the hooks (`useQuestSelection/Interactions/Animations/Data`) and currently passes via `CI=true npm test -- --runTestsByPath src/features/quest-board/hooks/__tests__/*`)._
...
- [x] **Status cue fix (major):** Update `SideQuestList` to apply a dedicated `.completed` class + theme tokens for finished side quests instead of reusing the `.started` style. Adds the CSS hooks needed so done vs. in-progress states stay visually distinct after the layout refactor. _`client/src/components/quest-card/SideQuestList.jsx` now emits `data-status` + `.completed`, while `client/src/index.css` and `client/src/App.css` expose theme-aware styling for the done state._
