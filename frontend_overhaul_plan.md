...
- [ ] **Follow-up · Hook split enforcement (urgent):** Finish extracting `useQuestSelection`, `useQuestInteractions`, and `useQuestAnimations` so `useQuests.js` only stitches their outputs together. The current 700+ line hook still owns selection, keyboard shortcuts, undo queues, and animation flags, so re-renders remain noisy until this split lands.
- [ ] **Follow-up · Hook-level tests:** Add RTL/Jest coverage for the new hooks (selection reset, undo timer expiry, layout refresh scheduling, collapse timers) and make sure the existing `useQuestData` tests stay green. These tests were part of the original acceptance criteria and should block the story from exiting.
...
- [ ] **Status cue fix (major):** Update `SideQuestList` to apply a dedicated `.completed` class + theme tokens for finished side quests instead of reusing the `.started` style. Adds the CSS hooks needed so done vs. in-progress states stay visually distinct after the layout refactor.
